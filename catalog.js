const productsByCategory = {
    chambers: [
        {
            id: 1,
            name: "Камера дезинфекционная ВФЭ-2/0,9 СЗМО",
            description: "Современная дезинфекционная камера для обработки медицинского инструментария и оборудования.",
            image: 'vfe209.png',
            link: "product-vfe-2-0-9.html",
            specifications: {
                volume: 0.9,
                power: 3.5,
                temperature: "100-120",
                dimensions: "1200×800×1800",
                weight: 250,
                price: "индивидуально"
            }
        },
        {
            id: 2,
            name: "Камера дезинфекционная ВФС 2/1,4 СЗМО",
            description: "Профессиональное оборудование для дезинфекции с расширенной камерой обработки.",
            image: 'images/vfs-2-1-4.jpg',
            link: "product-vfs-2-1-4.html",
            specifications: {
                volume: 1.4,
                power: 4.2,
                temperature: "100-125",
                dimensions: "1400×900×2000",
                weight: 320,
                price: "индивидуально"
            }
        },
        {
            id: 3,
            name: "Камера дезинфекционная КВФ-3/2,1 СЗМО",
            description: "Инновационная камера для дезинфекции с увеличенной камерой обработки.",
            image: 'images/kvf-3-2-1.jpg',
            link: "product-kvf-3-2-1.html",
            specifications: {
                volume: 2.1,
                power: 5.8,
                temperature: "100-130",
                dimensions: "1600×1000×2200",
                weight: 450,
                price: "индивидуально"
            }
        }
    ],
    sterilizers: [
        {
            id: 4,
            name: "Стерилизатор паровой ГК-100 СЗМО",
            description: "Паровой стерилизатор объемом 100 литров с автоматическим управлением.",
            image: 'images/gk-100.jpg',
            link: "product-gk-100.html"
        },
        {
            id: 5,
            name: "Стерилизатор ГК-50 СЗМО",
            description: "Компактный паровой стерилизатор объемом 50 литров.",
            image: 'images/gk-50.jpg',
            link: "product-gk-50.html"
        },
        {
            id: 6,
            name: "Стерилизатор ГК-25 СЗМО",
            description: "Настольный паровой стерилизатор для небольших объемов.",
            image: 'images/gk-25.jpg',
            link: "product-gk-25.html"
        }
    ],
    equipment: [
        {
            id: 7,
            name: "Дезинфекционная установка УД-100",
            description: "Мобильная установка для дезинфекции помещений и оборудования.",
            image: 'images/ud-100.jpg',
            link: "product-ud-100.html"
        },
        {
            id: 8,
            name: "Ветеринарное оборудование",
            description: "Профессиональный сушильный шкаф для медицинских инструментов.",
            image: 'images/shc-80.jpg',
            link: "product-shc-80.html"
        },
        {
            id: 9,
            name: "Ламинарный Бокс 1",
            description: "Автоматический дистиллятор для получения очищенной воды.",
            image: 'images/d-25.jpg',
            link: "product-d-25.html"
        }
    ]
};
